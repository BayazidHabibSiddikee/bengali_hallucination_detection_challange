...
# (Your get_category, is_math_or_logic, and build_sys_prompt remain perfectly preserved here)

def run_llm_subengine(df, model_title, path_or_id, use_prequant=False):
    if not cfg.use_llm_judge: return np.full(len(df), np.nan)
    
    # Run a deep memory purge before setting up the next heavyweight structure
    gc.collect(); torch.cuda.empty_cache()
    try: ctypes.CDLL("libc.so.6").malloc_trim(0)
    except: pass
    
    print(f"\n🚀 Engaging Pipeline Segment: Loading {model_title}...")
    tk = AutoTokenizer.from_pretrained(path_or_id)
    
    if use_prequant:
        # Pull your prequantized model exactly as specified in your baseline code
        llm = AutoModelForCausalLM.from_pretrained(path_or_id, device_map={"": 0}).eval()
    else:
        # Load standard Qwen fallbacks on-the-fly using 4-bit quant
        ...
        llm = AutoModelForCausalLM.from_pretrained(path_or_id, quantization_config=bnb, device_map={"": 0}).eval()

    dev = next(llm.parameters()).device
    ids1 = [tk.encode("1", add_special_tokens=False)[-1]]
    ids0 = [tk.encode("0", add_special_tokens=False)[-1]]
    out = np.zeros(len(df))
    ...
    for i, r in enumerate(df.itertuples()):
        ...
        enc = tk.apply_chat_template([{"role": "system", "content": SYS}, {"role": "user", "content": u}], add_generation_prompt=True, return_tensors="pt", return_dict=True)
    ...
    del llm, tk; gc.collect(); torch.cuda.empty_cache()
    return out

# Execute Sequential Segment A: Prequantized TigerLLM-9B-it
PREQUANT_PATH = "/kaggle/input/datasets/bayazidhs/tigerllm-9b-4bit/tigerllm-9b-4bit"
tiger_val = run_llm_subengine(sample, "TigerLLM-9B-it [Prequantized]", PREQUANT_PATH, use_prequant=True)
tiger_test = run_llm_subengine(test, "TigerLLM-9B-it [Prequantized]", PREQUANT_PATH, use_prequant=True)

# Execute Sequential Segment B: Multilingual Qwen Reasoning Brain
qwen_val = run_llm_subengine(sample, "Qwen2.5-3B-Instruct", "Qwen/Qwen2.5-3B-Instruct", use_prequant=False)
qwen_test = run_llm_subengine(test, "Qwen/Qwen2.5-3B-Instruct", "Qwen/Qwen2.5-3B-Instruct", use_prequant=False)

# Mix decisions evenly into the absolute master judge feature
